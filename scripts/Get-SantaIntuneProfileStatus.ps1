...
if (-not $context -or $context.TenantId -ne $TenantId -or $requiredScope -notin $context.Scopes) {
    Connect-MgGraph -TenantId $TenantId -Scopes $requiredScope -ContextScope CurrentUser -NoWelcome
...
$report = [ordered]@{
    ...
    pilotGroup = [ordered]@{ id = [string] $receipt.pilotGroup.id; displayName = [string] $receipt.pilotGroup.displayName }
    profiles = @()
}
